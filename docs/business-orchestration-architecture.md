...
- QuantumBPM：偏 BPMN、企业流程、审批和传统业务流程管理。
- Dify：业务人员和产品人员更熟悉，画布交互体验值得参考，但不需要复刻其 HTTP、IF/ELSE、循环等细粒度控制节点。
- 自研 React Flow 画布：可借鉴 Dify 的交互方式，同时吸收 QuantumBPM 的简洁流程设计理念，自己定义节点、DSL 和执行语义。

如果目标是让业务人员以更自然的方式编排业务能力，而不是学习 BPMN 规范，那么更适合采用 Dify 风格的交互体验，但节点抽象应更接近 QuantumBPM 的简洁业务流程设计，而不是 Dify 的细粒度工具链节点体系。
...
QuantumBPM+Temporal 这类方案对这些问题的解决思路是：
...
本方案不追求复刻 Dify 的完整节点体系，尤其不需要把 HTTP 请求、IF/ELSE、FOR 循环、异常分支、变量赋值等细粒度控制节点作为主要能力暴露给业务人员。更合适的方向是吸收 QuantumBPM 的简洁流程设计理念：画布只保留高层业务动作、少量业务判断、等待、人工任务和子流程。

换句话说，Dify 更适合作为交互体验参考，QuantumBPM 更适合作为流程抽象简洁性的参考。
...
子流程可以映射为 Temporal Child Workflow。

#### 5.5.1 Structural 元素选择

QuantumBPM / BPMN 中的 Sub-process、Child Workflow、Participant 都属于更偏结构化的建模元素。这里统一使用 Child Workflow 这个更贴近 Temporal 的命名，避免引入额外 BPMN 术语。

这些 structural 元素不是“做一件事”的普通 Task，而是在表达流程边界、复用边界、协作边界或一组任务的组织方式。

推荐理解如下：

| Structural 元素 | 中文理解 | 语义 | 典型场景 | Temporal / 平台映射 |
| --- | --- | --- | --- | --- |
| Sub-process | 内嵌子流程 | 当前流程内部的一组步骤，有明确入口、出口和顺序关系 | 把“执行一轮外呼”展开成准备名单、发起外呼、等待结果、汇总结果 | 可在同一个 Interpreter 中展开执行；复杂时也可编译成内部 scope |
| Child Workflow | 子 Workflow / 复用流程 | 调用一个独立、已发布的流程定义，调用方只关心输入输出 | 多个流程都复用“客户身份核验”“完整外呼子流程”“风控审批流程” | 映射为 Temporal Child Workflow，子流程有独立版本、历史和治理 |
| Participant | 参与者 / Pool | 表示流程协作中的组织、系统或角色边界，不代表一个可执行步骤 | 客户、销售系统、外呼平台、运营团队分别作为参与者 | 主要用于建模和权限/责任边界；不直接映射为 Activity |

这三者最容易混淆，核心区别可以用一句话区分：

```text
Sub-process    -> 当前流程内部展开一段流程
Child Workflow -> 调用另一个独立流程
Participant    -> 标注谁参与流程，不是执行节点
```

Sub-process 和 Child Workflow 的区别最重要：
...
- 第一阶段：跨流程复用统一叫 Child Workflow，运行时可在后续映射为 Temporal Child Workflow。
- Participant 主要用于画布展示、权限和责任边界，不参与 Execution Plan 的节点推进。
- 稳定、可复用、跨流程共享的能力优先沉淀为 Child Workflow；仅在当前流程内部复用的局部步骤优先用 Sub-process。

### 5.6 Task 类型语义与支持策略

QuantumBPM / BPMN 中会区分 Generic Task、Service Task、User Task、Manual Task、Send Task、Receive Task、Script Task、Business Rule Task。这些 Task 类型不是为了增加画布复杂度，而是为了让流程图表达“这一步工作的业务性质”。
...
因此，本平台可以吸收 BPMN / QuantumBPM 的 Task 语义，但不必一开始完整实现所有 BPMN Task。
...
这能保留 QuantumBPM / BPMN 的清晰语义，同时避免把完整 BPMN 复杂度一次性搬进平台。
...
如果选择 Dify 风格的自研业务画布，则 QuantumBPM 与自研画布处在同一层，通常不需要同时使用。

但 QuantumBPM 的简洁流程抽象值得吸收：
...
后续可以把当前 BPMN / QuantumBPM 映射思路升级为：
