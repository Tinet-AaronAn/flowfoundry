...
**生命周期**：平台 namespace 由管理员（Namespaces 页 / Admin API）创建，或由平台启动时的 `NamespaceBootstrapRunner` 按当前业务 Registry 做 `ensureRegistered`。App Worker / SDK **不能**自助新增平台 namespace；Worker 上报的 DeploymentContract 只声明「在该 namespace 上运行」。App 开发前置见 [workflow-development-guide.md §3.5](./workflow-development-guide.md#35-namespace-前置条件必读)。
