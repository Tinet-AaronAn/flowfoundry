...
- Temporal 的 `namespace`（物理隔离单位，示例业务模块当前为 `call-campaign`）；
...
平台默认配置在 `flowfoundry-core/.../application-flowfoundry-platform.yml`（数据源、Redis、Temporal 默认值等）；各场景 `application.yml` 通过 `spring.config.import` 引入并覆盖 namespace、task-queue、注册表路径等。
...
## 11. Namespace 体系设计（目标设计）

> 本章为**目标设计**，用于统一 namespace 概念、指导后续实现；文末「11.6 现状与差距」明确当前代码尚未达成的部分。

### 11.1 问题：两个「namespace」被混用

当前代码里有两处都叫 `namespace`，但语义完全不同，是本次梳理的根源：

| 名称 | 出处 | 真实含义 |
| ---- | ---- | -------- |
| `temporal.namespace` | `TemporalProperties`、`FlowFoundryPlatformConfiguration` | **Temporal 服务端的物理隔离单位**（workflow 执行、run logs / history 落在哪个 namespace） |
| `workflow_definition.namespace` == `tenantId` | `NamespaceAccessService`、`WorkflowDefinitionEntity`、`flowfoundry.security.dev-namespace` | **逻辑租户标识**，仅用于 RBAC / 流程定义归属 |

问题不止是同名。看启动链路 `FlowController.run()`：

- `workflowClient` 绑死在**单一** `temporal.namespace`，task-queue 用平台自己的 `temporalProperties.taskQueue()`；
- `tenantId` 只用于拼 `businessKey` 前缀和做访问控制，**完全不影响 workflow 执行和 run logs 落在哪个 Temporal namespace**。

结果：**所有使用方（所有 flowfoundry-app 模块）的 workflow 执行与 run logs 全部挤在同一个 Temporal namespace 里**，无法做到「每个使用方独立、互不可见」。

### 11.2 两个概念彻底解耦

本设计明确：**Temporal namespace（物理）与 tenantId（逻辑）完全解耦，各自独立配置**，取值互不约束。

| 维度 | Temporal namespace | tenantId |
| ---- | ------------------ | -------- |
| 定位 | 物理执行 / 隔离边界 | 逻辑归属 / RBAC |
| 隔离级别 | 强隔离：workflow + run logs + schedule 互不可见 | 平台内授权可见性 |
| 取值 | 由使用方（flowfoundry-app 模块）自定：`tenantId` / `appId` / 二者组合，平台不写死语义，仅作为**不透明路由 key** | `workflow_definition.namespace`，建模 / 导入时确定 |
| 传递 | 由 app 部署契约声明（见 11.4） | 请求头 `X-Tenant-Id`（兼容 `X-Platform-Namespace`） |

两者可以相同（一个租户一个物理 namespace），也可以是「一个物理 namespace 内含多个 tenantId」，由使用方自行决定，平台不做强制映射。

### 11.3 目标隔离模型：一个系统 namespace + 每使用方一个业务 namespace

```
┌──────────────────────────────────────────────────────────────┐
│ Temporal Server                                                │
│                                                                │
│  flowfoundry-system      ← 固定系统 namespace（flowfoundry-core）│
│     · 平台内部编排 / 运维类工作流（当前预留，基本为空）           │
│     · 纯管理操作（建模器 / API Key / 审计）本就不接触 Temporal    │
│                                                                │
│  <app-A namespace>       ← 使用方 A（flowfoundry-app 模块 A）    │
│     · workflow 执行 · run logs · timer 调度 · human task · 子流程 │
│                                                                │
│  <app-B namespace>       ← 使用方 B（与 A 完全物理隔离）          │
│     · ……                                                       │
└──────────────────────────────────────────────────────────────┘
```
...
1. **凡是使用方触发的生产运行** → 落到该使用方自己的业务 namespace（workflow 执行、run logs、timer 调度、human task、子工作流）。
2. **flowfoundry-core 后台建模器发起的调试运行**（`runSource=web-modeler`）→ 落到固定的 `flowfoundry-system`，与业务生产 run logs 隔离。建模器调试使用桩 Activity，但**桩化发生在 Activity 内部**，活动仍会派发到业务 Task Queue，因此使用方 Worker 需**同时在业务 namespace 与系统 namespace 上各起一个 Worker（同 Task Queue、同注册）**；平台按 `runSource` 选择目标 namespace，两者 Task Queue 相同。API Key / 审计等纯管理面本就不碰 Temporal。
3. **namespace 值由 app 决定**，平台当作不透明 key 路由，不假设它等于 tenantId。

### 11.4 App 部署契约与自动注册

每个 flowfoundry-app 模块（Worker，`run-mode=worker`）在其 `application.yml` 中已声明自己的运行契约：

```yaml
temporal:
  namespace: ${TEMPORAL_NAMESPACE:call-campaign}     # 该使用方的物理 namespace
  task-queue: ${TEMPORAL_TASK_QUEUE:ai-collection-strategy}
```

**部署契约（Deployment Contract）** = `{ appId, registryNamespace, temporalNamespace, taskQueue }`（`DeploymentContract`）。

**自动注册流程（已实现）**：Worker（`run-mode=worker`）启动（`ApplicationReadyEvent`）时由 `WorkerContractPublisher` 把契约写入共享 Redis 键 `flowfoundry:contract:<registryNamespace>`（TTL 90s），并每 30s 心跳续期；平台侧 `DeploymentContractRegistry` 读取，据此**按 namespace 懒加载并缓存 `WorkflowClient` / `ScheduleClient`（`TemporalClients`）**。

选择 **共享 Redis** 而非 HTTP 注册端点的原因：Worker 无需反向依赖平台地址、不受启动顺序影响、无需为注册端点单独处理鉴权；平台与 Worker 均已具备 `StringRedisTemplate`。

**回退**：当尚无 Worker 注册时，平台用 `DeploymentContractRegistry.localContract()`（本地加载的 Activity 注册表 `namespace` / `defaultTaskQueue` + `temporal.namespace`）作为默认契约，保证单场景部署开箱可用。`resolveForRun()` 优先匹配已注册的同 `registryNamespace` 契约，否则回退本地契约。

### 11.5 路由改造要点（实现清单）

| 环节 | 现状 | 目标 |
| ---- | ---- | ---- |
| 客户端 | `FlowFoundryPlatformConfiguration` 建**单个** `WorkflowClient`/`ScheduleClient` 绑死单 namespace | 改为**按 namespace 缓存的 client 工厂/注册表** |
| 启动 run | `FlowController.run()` 用全局 namespace + 平台 task-queue | 解析目标业务 namespace + 该 app 的 task-queue 后取对应 client 启动 |
| 查询 run 状态 / logs | `FlowRunStatusService` 无脑用全局 `temporalProperties.namespace()` | 先由 `workflowId`/run 记录/定义反查所属 namespace，再查该 namespace |
| Timer 调度 | `StartTimerScheduleService` 用全局 namespace | 用流程所属业务 namespace |
| Human Task | `completeHumanTask` 用全局 client | 用 run 所属 namespace 的 client |
| 命名 | `namespace` 一词双关 | 代码/配置区分 `temporalNamespace`（物理）与 `tenantId`（逻辑） |

### 11.6 落地实现摘要

已按本设计落地（flowfoundry-core）：

- 新增 `flowfoundry.namespace.system`（默认 `flowfoundry-system`）区分系统 namespace。
- `TemporalClients`：按 namespace 缓存 `WorkflowClient` / `ScheduleClient`，共享单一 `WorkflowServiceStubs`；取代原绑死单 namespace 的单例 bean。
- `DeploymentContract` / `DeploymentContractRegistry`：Redis 共享的使用方契约注册与解析（含本地回退）。
- `WorkerContractPublisher`（仅 `run-mode=worker`）：启动上报契约 + 心跳续期。
- `RunNamespaceLocator`（仅平台）：run 启动时记录 `workflowId→namespace`；查询时先读缓存、未命中跨已知业务 namespace 扫描并回填。
- `FlowController.run()` / `completeHumanTask`、`FlowRunStatusService`、`StartTimerScheduleService` 均改为按解析出的业务 namespace + taskQueue 路由。其中 `FlowController.run()` 对 `runSource=web-modeler` 的调试运行改路由到 `flowfoundry-system`；`RunNamespaceLocator` 的跨 namespace 扫描已纳入系统 namespace，保证调试运行状态/日志可被查询。
- `TemporalWorkerBootstrap` 改为 `@ConditionalOnFlowFoundryWorker`：平台不再运行业务 Worker，仅作为按 namespace 的客户端。使用方 Worker 在**业务 namespace + 系统 namespace 两个 namespace** 上各起一个 Worker（同 Task Queue、同解释器 + Activity 注册），分别承载生产运行与后台建模器调试运行。
- `deploy/docker-compose.local.yml` 的 `temporal-init` 额外创建 `flowfoundry-system` namespace；生产部署需预先创建该系统 namespace。

保持不变：

- tenantId / RBAC 语义不变，仅与物理 namespace 解耦。
- 单场景本地开发（platform + 单一 app Worker，同 `call-campaign` namespace）行为不变。