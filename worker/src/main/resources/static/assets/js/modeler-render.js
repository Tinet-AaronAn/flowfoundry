      async function loadActivities() {
        try {
          ...
          message(t('message.loadActivitiesFailed', { error: err.message }));
        }
      ...
      function glyph(kind) {
        ...
        if (kind === 'businessRuleTask') return '<span class="glyph script">𝑓</span>';
      ...
      function renderCanvas() {
        ...
        const highlightId = state.simulation?.currentNodeId || state.runtimeHighlightNodeId;
        for (const current of nodes) {
          ...
          el.className = `node ${nodeClass(current)} ${selected ? 'selected' : ''} ${simActive ? 'simulation-active' : ''} ${state.connectionSource === current.id ? 'connecting-source' : ''}`;
      ...
      function nodeMeta(current) {
        ...
        if (['userTask', 'humanTask', 'manualTask'].includes(current.kind)) {
      ...
      function renderNodeProperties() {
        ...
        $('properties').innerHTML = `
          ...
          <div class="prop-section"><h3>Actions</h3>
            ...
            <div class="help">${escapeHtml(t('prop.deleteNodeHelp'))}</div>
          </div>
      ...
      function taskDefinitionSection(n) {
        if (!['task','serviceTask','sendTask','scriptTask','businessRuleTask'].includes(n.kind)) return '';
      ...
      function humanTaskMode(n) {
        return n.config?.flowFoundryHumanTask?.mode
          || (n.kind === 'manualTask' ? 'offline' : 'managed');
      }

      function humanTaskSection(n) {
        if (!['userTask', 'humanTask', 'manualTask'].includes(n.kind)) return '';
      ...
      function scriptSection(n) {
        if (n.kind !== 'scriptTask' && n.kind !== 'businessRuleTask') return '';
      ...
      function receiveTaskSection(n) {
        ...
        return `<div class="prop-section"><h3>Receive Task</h3>
          ...
          <div class="help">${escapeHtml(t('prop.receiveTaskHelp'))}</div>
        </div>`;
      }
      ...
      function taskMorphTypes() {
        return [
          ...
          { kind: 'userTask', label: 'Human Task', config: { flowFoundryHumanTask: { mode: 'managed' }, flowFoundryAssignmentDefinition: { candidateGroups: 'operator' } } },
          ...
          { kind: 'scriptTask', label: 'Script Task', activityType: 'dmn-decision', decisionRef: 'demo-script', decisionVersion: '1.0.0' }
        ];
      }
      ...
      async function createNodeFromPaletteItem(item, point = null, options = {}) {
        ...
        if (item.kind === 'scriptTask' || item.kind === 'businessRuleTask') {
          n.activityType = 'dmn-decision';
