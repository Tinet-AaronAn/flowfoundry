...
public record FlowFoundryTrace(
  ...
  public static FlowFoundryTrace fromNode(ExecutionNode node) {
    ...
    String fallbackName = node.id();
    String fallbackActivity = node.activityType();
    if (node.config() != null) {
      Object childName = node.config().get("childWorkflowName");
      if (childName != null && !String.valueOf(childName).isBlank()) {
        fallbackName = String.valueOf(childName).trim();
      } else {
        Object childId = node.config().get("childWorkflowId");
        if (childId != null && !String.valueOf(childId).isBlank()) {
          fallbackName = String.valueOf(childId).trim();
        }
      }
    }
    if (blank(fallbackActivity) && node.kind() == NodeKind.CHILD_WORKFLOW) {
      fallbackActivity = "child-workflow";
    }
    return new FlowFoundryTrace(node.id(), fallbackName, null, fallbackActivity);
  }
